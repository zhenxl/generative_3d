...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        if self.is_isotropic:
            ...
            s_mats = torch.diag_embed(scales.repeat(1,3))
        ...
        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
            ...
            ### YOUR CODE HERE ###
            r_mats = quaternion_to_matrix(quats) # N,3,3
            s_mats = torch.diag_embed(scales)
            cov_3D = r_mats @ s_mats @ s_mats.transpose(1,2) @ r_mats.transpose(1,2)  # (N, 3, 3)
    ...
    def evaluate_gaussian_2D(points_2D: torch.Tensor, means_2D: torch.Tensor, cov_2D_inverse: torch.Tensor):
        ...
        power = -0.5 * diff.transpose(-2, -1) @ cov_2D_inverse @ diff 
        power = power.squeeze(-1).squeeze(-1)  # (N, H*W)
...
class Scene:
    ...
    def compute_depth_values(self, camera: PerspectiveCameras):
        ...
        view_transform = camera.get_world_to_view_transform()
        means_2d = view_transform.transform_points(means_3d)
        z_vals = means_2d[:,2]  # (N,)
    ...
    def get_idxs_to_filter_and_sort(self, z_vals: torch.Tensor):
        ...
        """
        ### YOUR CODE HERE ###
        n_z_vals, z_idx = torch.sort(z_vals)
        z_idx = z_idx[n_z_vals >= 0]
        z_idx = z_idx.to(torch.int64)
    ...
    def calculate_gaussian_directions(self, means_3D, camera):
        ...
        gaussian_dirs = camera.get_world_to_view_transform().transform_points(means_3D)  # (N, 3)
        gaussian_dirs_norm = torch.nn.functional.normalize(gaussian_dirs)
